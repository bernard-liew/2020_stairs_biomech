...
df_plot <- df %>%
  ...
  group_by(study, speed_cat, age_cat, joint, cycle) %>%
  summarize (val.mean = mean(val, na.rm = TRUE),
             val.sd = sd(val, na.rm = TRUE),
             val.skew = e1071::skewness(val),
             val.kurt = e1071::kurtosis(val)) %>%
  ungroup () %>%
...
f <- ggplot (data = df_plot) +
  geom_line (aes (x = cycle, y = val, colour = study, linetype = speed_cat)) +
  guides(colour = guide_legend(title="Speed (m/s)")) +
  facet_wrap_paginate(age_cat~joint + var, ncol = 4, nrow = 2, scales = "free")
...
for (i in 1:n) {
  ...
  print (f + facet_wrap_paginate(var+joint ~  age_cat, ncol = 4, nrow = 2, page = i, scales = "free"))
...
temp3$cycle <- parse_number(temp3$cycle)
temp3$subj <- parse_number(temp3$cycle)
