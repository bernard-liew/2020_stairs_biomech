...
```{r}
m_in_cm <- function(x) if(x<100) x * 100 else x
...
saveRDS(df_ankle, file.path(output_dir, "df_ankle.RDS"))
saveRDS(df_knee, file.path(output_dir, "df_knee.RDS"))
saveRDS(df_hip, file.path(output_dir, "df_hip.RDS"))
...
df.summary <- df %>%
  ...
  slice (1) %>%
  ungroup()%>%
  dplyr::select (-c(val, cycle, joint, subj))

  
summary (tableby(study ~., data = df.summary), text = TRUE)
...
```{r}
form_right = paste(c("~ ba (~ te(cycle, speed)",
                     "te(cycle, age)",
                     "te(age, speed)", 
...
models <- mclapply(fams, function(fam)
  m1 <- try(gamlss(as.formula(paste0("val ", form_right)),
...
saveRDS(models, file.path(output_dir, "ankle_gamlss_models.RDS"))
rm(models)
gc()
...
saveRDS(models, file.path(output_dir, "knee_gamlss_models.RDS"))
...
saveRDS(models, file.path(output_dir, "hip_gamlss_models.RDS"))

dev_knee <- sapply(readRDS(file.path(output_dir, "knee_gamlss_models.RDS")), "[", "G.deviance")
dev_ankle <- sapply(readRDS(file.path(output_dir, "ankle_gamlss_models.RDS")), "[", "G.deviance")
dev_hip <- sapply(readRDS(file.path(output_dir, "hip_gamlss_models.RDS")), "[", "G.deviance")

res <- data.frame(
  distribution = fams,
  knee = unlist(dev_knee),
  ankle = unlist(dev_ankle),
  hip = unlist(dev_hip)
)

saveRDS(res, file.path(output_dir, "final_result.RDS"))

best_model_knee <- gamlss(as.formula(paste0("val ", form_right)),
                          sigma.fo = as.formula(form_right2),
                          family = "SST",
                          data = df_knee,
                          trace = FALSE)

saveRDS(best_model_knee, file.path(output_dir, "best_model_knee.RDS"))

best_model_ankle <- gamlss(as.formula(paste0("val ", form_right)),
                         sigma.fo = as.formula(form_right2),
                         family = "NET",
                         data = df_ankle,
                         trace = FALSE)

saveRDS(best_model_ankle, file.path(output_dir, "best_model_ankle.RDS"))

best_model_hip <- gamlss(as.formula(paste0("val ", form_right)),
                         sigma.fo = as.formula(form_right2),
                         family = "EGB2",
                         data = df_hip,
                         trace = FALSE)

saveRDS(best_model_hip, file.path(output_dir, "best_model_hip.RDS"))
...
models <- vector("list", length(fams))
