...
# Export
pdf (width = 12, height = 8, file = "manuscript/fig1.pdf")
...
peaks <- hsmo[hsmo$fit == max (hsmo$fit),]
f <- peaks[,2]
lcl <- f - 1.96 * peaks[,3]
ucl <- f + 1.96 * peaks[,3]
age <- peaks[,1]
...
ucl <- f + 1.96 * peaks[,3]
age <- peaks[,1]
cat ("The min value of hip is ", round (f, 2), "(95%CI", round (lcl, 2), "to", round (ucl, 2), ")", "which occurs at", round (age, 0), "(years)")
...
pp_ank <- get_predictions(smo, 
  ...
  mutate (age = factor (age),
          speed = factor (speed, labels = c("1m/s", "1.5m/s")))
...
```{r, eval = FALSE, include = FALSE}
# Export
pdf (width = 6, height = 7, file = "manuscript/fig4.pdf")
