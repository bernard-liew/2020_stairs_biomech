...
```{r}
# Height
f1 <- ggplot(dat_sub, aes(x = age, ht)) +
      geom_point () + 
      ...
      theme_cowplot() 
# Weight
f2 <- ggplot(dat_sub, aes(x = age, wt)) +
      geom_point () + 
      geom_smooth(method = "gam", formula = y ~ s(x, bs = "cr")) + 
      labs (x = "Age (yo)",
            y = "Weight (kg)") +
      theme_cowplot() 
# Speed
f3 <- ggplot(dat_sub, aes(x = age, speed)) +
      geom_point () + 
      geom_smooth(method = "gam", formula = y ~ s(x, bs = "cr")) + 
      labs (x = "Age (yo)",
            y = "Speed (m/s)") +
      theme_cowplot()
# Stride length
f4 <- ggplot(dat_sub, aes(x = age, strlen)) +
      geom_point () + 
      geom_smooth(method = "gam", formula = y ~ s(x, bs = "cr")) + 
      labs (x = "Age (yo)",
            y = "Stride length (m)") +
...
f5 <- dat_sub %>%
  ...
  summarize (age_mean = mean (age),
          age_sd = sd (age)) %>%
  ggplot() +
      geom_col (aes (x = sex, y = age_mean)) +
      geom_errorbar(aes (x = sex, ymin = age_mean - age_sd, ymax  = age_mean + age_sd), width = 0.1) + 
      labs (x = "Sex",
            y = "Age (yo)") +
      theme_cowplot() 
...
pdf (width = 8, height = 8, file = "manuscript/fig1.pdf")
...
pdf (width = 8, height = 8, file = "manuscript/fig3.pdf")
