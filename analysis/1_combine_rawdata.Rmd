...
raw_dir <- "../../mocap_database/data_stairs"
...
temp <- df_restruct %>%
  ...
  filter (task == "walking") %>%
  mutate (joint = ifelse (var_axis == "anklepwr", "ankle",
  ...
  ungroup () %>%
  # Retains all speeds
  group_by(subj, joint, cond, speed, cycle) %>%
  summarize (age = age[1],
             sex = sex[1],
             ht = ht[1],
             wt = wt[1],
...
saveRDS(temp, file = file.path(output_dir, file_name))
```
...
raw_dir <- "../../mocap_database/data_taylor"
...
df <- df.list %>%
  ...
  rename (sex = Gender,
          ...
          stplen = `Step Length`,
          speed = `Walking Speed`) %>%
  mutate (joint = ifelse (grepl ("Knee", var), "knee",
                          ifelse (grepl ("Hip", var), "hip", "ankle")),
          cond = "self") %>%
  group_by(subj, joint, cond, speed, cycle, strlen, stplen) %>%
  summarize (age = age[1],
           sex = sex[1],
           ht = ht[1],
           wt = wt[1],
...
raw_dir <- "../../mocap_database/data_fukuchi/fun"

list_of_files <- list.files(raw_dir, pattern = glob2rx("*walkT*.txt")) #"*T05*.txt"
...
vel_dir <- "../../mocap_database/data_fukuchi/scalar"

list_of_files_vel <- list.files(vel_dir, pattern = "*Step|*Stride")
...
vel <- vel.list %>%
  bind_rows(.id = "fname") %>%
  mutate (fname = str_remove (fname, "_bad"),
          subj_cond = str_extract_all (fname, ".*T[[:digit:]]{2}"),
          variab = str_remove_all (fname, ".*_walkT|.txt|_Mean|[[:digit:]]+_")) %>%
  separate(subj_cond, into = c("subj", "cond"), sep = "_") %>%
  dplyr::select (-c(ITEM, fname)) %>%
  pivot_wider(names_from = variab,
              values_from = X) %>%
  mutate_if(is.character, tolower) %>%
  mutate (stplen = (Left_Step_Length + Right_Step_Length)/2)
...
raw_dir <- "../../mocap_database/data_1/v3d/data/fun"
...
vel_dir <- "../../mocap_database/data_1/v3d/data"

list_of_files_vel <- list.files(vel_dir, pattern = "*VEL|*Step|*Stride")
...
demo <- readxl::read_excel ("../../mocap_database/data_1/Gait_subject_info.xlsx") 
...
vel <- vel.list %>%
  bind_rows(.id = "fname") %>%
  mutate (subj_cond = str_remove_all (fname, "_Gait.*|_COM.*"),
          variab = str_remove_all (fname, ".*_Gait_|.*_COM_|.txt|_Mean")) %>%
  separate(subj_cond, into = c("subj", "cond"), sep = "_") %>%
  dplyr::select (-c(ITEM, fname)) %>%
  pivot_wider(names_from = variab,
              values_from = X) %>%
...
raw_dir <- "../../mocap_database/data_2/v3d/data/fun"
...
# Import demography data, 0: woman, 1: man

demo_dir <- "../../mocap_database/data_2/v3d/data/scalar"

list_of_files_demo <- list.files(demo_dir, pattern = "*DEMO.txt")

registerDoParallel(cores = 6)

demo.list  <- foreach(f = 1:length(list_of_files_demo), .packages="tidyverse")%dopar% {

  read_tsv (file = file.path (demo_dir, list_of_files_demo [f]), 
                  col_names = T, 
                  skip = 4)
}

names (demo.list) <- list_of_files_demo

# Import speed data

vel_dir <- "../../mocap_database/data_2/v3d/data/scalar"
...
## modify demography and velocity data

```{r}

demo_df <- demo.list  %>%
  map (function (x) {x$ITEM = NULL; return (x)}) %>%
  bind_rows(.id = "demo") %>%
  #dplyr::select (-ITEM) %>%
  rename (speed = `0`,
          age = `1`,
          ...
          wt = `3`,
          sex = `4`) %>%
  mutate (demo = str_remove(demo, "_DEMO.txt")) %>%
  separate(demo, into = c("subj", "cond", "trial"), sep = "_") %>%
  mutate_if(is.character, tolower)

vel <- vel.list %>%
  bind_rows(.id = "fname") %>%
  mutate (subj_cond = str_remove_all (fname, "_Left.*|_Right.*|_Stride.*"),
          variab = str_extract_all (fname, "Left.*|Right.*|Stride.*"),
          variab = str_remove_all (variab, "_Mean.*|.txt")) %>%
  separate(subj_cond, into = c("subj", "cond", "trial"), sep = "_") %>%
  dplyr::select (-c(ITEM, fname)) %>%
  pivot_wider(names_from = variab,
              values_from = X) %>%
  mutate_if(is.character, tolower) %>%
...
df3.list  <- foreach(f = 1:length(df2.list), .packages="tidyverse")%dopar% {
  ...
  # split identifier
 id.df <- data.frame (id = id2[f]) %>%
   separate(id, into = c("subj", "cond", "trial", "side", "joint", "var"), sep = "_")
...
# condition cond == "c4" is self-paced
df4 <- df3.list %>%
  bind_rows () %>%
  inner_join(vel, by = c("subj", "cond", "trial")) %>%
  inner_join(demo_df, by = c("subj", "cond", "trial")) %>%
  # Retains all speeds
  group_by(subj, joint, cond, speed, cycle) %>%
  summarize (age = age[1],
             sex = sex[1],
